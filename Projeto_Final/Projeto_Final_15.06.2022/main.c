...
void Timer2A_Handler(void){
  ...
  TIMER2_ICR_R = TIMER_ICR_TATOCINT;		// acknowledge TIMER0A timeout
	TIMER2_ICR_R = 0x00000001;    				// clear TIMER0A timeout flag
	liga_pwm=0;
	//GPIO_PORTF_DATA_R = TABELA_CORES[i];	// acende LEDs	
	//i=(i+1)&0x07;													// mascara para contagem de 0 a 7
}

int main(void){ 
	int32_t In,Vin,cont=0;
	//int32_t lim_time = 1000;			// Tempo limite do dispensador
	uint8_t Estado = 1;				// Declara estado inicial
	char UART_data;
  Output_Init();              	// initialize output device
  PortA_Init();									// initialize ports a,b,c e f
	PortB_Init();
	PortC_Init();
	PortF_Init();
	...
	while(1){
		switch(Estado){
			case 1:
				GPIO_PORTF_DATA_R = 0x0E;			// acende todos os LEDs
			...
			case 5:
				...
				break;
			case 6:
				Vin = ADC0_InSeq3();						// Faz a leitura do ADC
				if(Vin >= valor_15cm){
					unidade = 7;
					dezena = 0;
					Estado = 7;
			...
			case 7:
				...
				liga_pwm = 1;
				/*
				In = GPIO_PORTF_DATA_R & 0X0F;	// Leitura da chave SW1 para definir a vazão
				if(In == 0x00){									// Teste se a chave está fechada
						unidade = 4;
						dezena = 0;
						Estado = 4;
				...
				*/
				Timer2_Init(tempo_limite);		// initialize timer2 com o parametro tempo limite
				unidade = 8;
				dezena = 0;
				Estado = 8;
				break;
			case 8:
				cont=cont+1;			//Incrementar um contador de número de acionamentos
				printf("\nLiberacao do alcool em gel habilitada");
				printf("\nNumero de acionamentos com liberacao de alcool em gel = %d",cont);
				unidade = 9;
				dezena = 0;
				Estado = 9;
				break;
			case 9:
				...
				break;
			case 10:
				liga_pwm=0;						//Desligar o sinal PWM
				TIMER2_CTL_R = 0x00; 	//Desliga o temporizador
				printf("\nLiberacao do alcool desabilitada devido ao tempo limite");
				unidade = 1;
				dezena = 1;
				Estado = 11;
				break;
